\TLV_version [\source warp-v_risc-v_1-stage.tlv] 1d --noline: tl-x.org

\source warp-v_risc-v_1-stage.tlv 6
\SV
   // Include WARP-V.
...
\source warp-v_risc-v_1-stage.tlv 10
\TLV
   \source ./warpv.tlv 3556   // Instantiated from warp-v_risc-v_1-stage.tlv, 12 as: m4+warpv()
      // =================
      //
      //    THE MODEL
      //
      ...
      \source ./warpv.tlv 2814   // Instantiated from warp-v_risc-v_1-stage.tlv, 3564 as: m4+cpu(/top)
         // Generated logic
         \source <builtin> 1   // Instantiated from warp-v_risc-v_1-stage.tlv, 2816 as: m4+indirect(M4_isa['_gen'])
         ...
         \source <builtin> 1   // Instantiated from warp-v_risc-v_1-stage.tlv, 2819 as: m4+indirect(M4_IMEM_MACRO_NAME, M4_PROG_NAME)
            \source ./warpv.tlv 1289   // Instantiated from built-in definition.
               \source <builtin> 1   // Instantiated from warp-v_risc-v_1-stage.tlv, 1290 as: m4+indirect(['riscv_']divmul_test['_prog'])
                  ...
                  \end_source
               \end_source
               \source ./warpv.tlv 3641   // Instantiated from warp-v_risc-v_1-stage.tlv, 1291 as: m4+instrs_for_viz()
         ...
         |fetch
            /instr
               ...
               @0
                  ...
                  \source <builtin> 1   // Instantiated from warp-v_risc-v_1-stage.tlv, 3013 as: m4+indirect(M4_isa['_decode'])
                     \source ./warpv.tlv 1498   // Instantiated from built-in definition.
                        ...
                        ?$valid_decode
                           ...
                           // Instruction decode.
                           \source ./warpv.tlv 1478   // Instantiated from warp-v_risc-v_1-stage.tlv, 1526 as: m4+riscv_decode_expr()
                     ...
                     \end_source
                  \end_source
               \source <builtin> 1   // Instantiated from warp-v_risc-v_1-stage.tlv, 3014 as: m4+indirect(['branch_pred_']M4_BRANCH_PRED)
                  ...
                  /src[2:1]
                     ...
                     ?$is_reg_condition
                        {$reg_value[31:0], $pending} =
                           ...
                           {/instr/regs[$reg]>>0$value, /instr/regs[$reg]>>0$pending};
               ...
               // Execute
               // =======
               \source <builtin> 1   // Instantiated from warp-v_risc-v_1-stage.tlv, 3099 as: m4+indirect(M4_isa['_exe'], @M4_EXECUTE_STAGE, @M4_RESULT_STAGE)
                  \source ./warpv.tlv 1641   // Instantiated from built-in definition.
                     // if M_EXT is enabled, this handles the stalling logic
                     
                     \source ./warpv.tlv 2570   // Instantiated from warp-v_risc-v_1-stage.tlv, 12 as: m4+m_extension()
                     ...
                     @0
                        ...
                        \source ./warpv.tlv 2607   // Instantiated from warp-v_risc-v_1-stage.tlv, 12 as: m4+warpv_mul(|fetch/instr,/mul1, $mulblock_rslt, $wrm, $waitm, $readym, $clk, $resetn, $mul_in1, $mul_in2, $instr_type_mul, $mulblk_valid)
                           /mul1      
                        ...
                        \end_source
                        \source ./warpv.tlv 2637   // Instantiated from warp-v_risc-v_1-stage.tlv, 12 as: m4+warpv_div(|fetch/instr,/div1, $divblock_rslt, $wrd, $waitd, $readyd, $clk, $resetn, $div_in1, $div_in2, $instr_type_div, >>1$div_stall)
                           /div1
                     ...
                     // CSR logic
                     // ---------
                     \source ./warpv.tlv 1394   // Instantiated from warp-v_risc-v_1-stage.tlv, 1894 as: m4+riscv_csrs((m4_csrs))
                        
                        \source ./warpv.tlv 1352   // Instantiated from warp-v_risc-v_1-stage.tlv, 12 as: m4+riscv_csr(m4_echo(['m4_csr_']csr['_args']))
                           //--------------
                           // CSR CYCLE
                           //--------------
                           @0
                     ...
                     \end_source
                     @0
                        \source ./warpv.tlv 1399   // Instantiated from warp-v_risc-v_1-stage.tlv, 1896 as: m4+riscv_csr_logic()
                     ...
                     @0
                        ...
                        \source ./warpv.tlv 1481   // Instantiated from warp-v_risc-v_1-stage.tlv, 1955 as: m4+riscv_rslt_mux_expr()
               ...
               @0
                  ...
                  // Normal case:
                  $commit = (! $reset && >>0$next_good_path_mask[1]) && ! $abort;
         ...
         \source ./warpv.tlv 2464   // Instantiated from warp-v_risc-v_1-stage.tlv, 3158 as: m4+fixed_latency_fake_memory(/top, 0)
   ...
   \end_source
   \source ./warpv.tlv 3196   // Instantiated from warp-v_risc-v_1-stage.tlv, 13 as: m4+warpv_makerchip_cnt10_tb()
      |fetch
         /instr
            @0
   ...
   \end_source
   \source ./warpv.tlv 4013   // Instantiated from warp-v_risc-v_1-stage.tlv, 14 as: m4+makerchip_pass_fail()
      |done
         @0
            // Assert these to end simulation (before Makerchip cycle limit).
