...
logic [15:0] Port_RG_Trans_cyc_cnt_a1 [3:0] /* verilator lint_save */ /* verilator lint_off MULTIDRIVEN */,
   ...
   for (port = 0; port <= 3; port++) begin : L1gen_Port
      ...
      //
         ...
         //
            ...
            // For $cyc_cnt.
            always_ff @(posedge clkP_Port_RG_valid_a2[port]) Port_RG_Trans_cyc_cnt_a2[port][15:0] <= Port_RG_Trans_cyc_cnt_a1[port][15:0];
      ...
      for (port = 0; port <= 3; port++) begin : L1gen_Tb_Port
         ...
         //
            ...
            //
               ...
               // For $cyc_cnt.
               always_ff @(posedge clkP_Tb_Port_RECEIVE2_avail_a1[port]) L1_Tb_Port[port].L1_RECEIVE2_Trans_cyc_cnt_a1[15:0] <= L1_Tb_Port[port].L1_RECEIVE2_Trans_cyc_cnt_a0[15:0];
               ...
               // For $sender.
               always_ff @(posedge clkP_Tb_Port_RECEIVE2_avail_a1[port]) L1_Tb_Port[port].L1_RECEIVE2_Trans_sender_a1[1:0] <= L1_Tb_Port[port].L1_RECEIVE2_Trans_sender_a0[1:0];
...
generate
   ...
   if (1) begin : DEBUG_SIGS
      ...
      // Scope: /port[3:0]
      //
      for (port = 0; port <= 3; port++) begin : \/port 
         ...
         if (1) begin : \|fifo_in 
            ...
            //
            if (1) begin : \/trans 
               logic [15:0] \>>?$avail>@1$cyc_cnt ;
               assign \>>?$avail>@1$cyc_cnt = L1e_Port[port].L1_FIFO_IN_Trans_cyc_cnt_a1;
               logic [7:0] \>>?$accepted>@1$data ;
               assign \>>?$accepted>@1$data = L1c_Port[port].L1_FIFO_IN_Trans_data_a1;
               logic [1:0] \>>?$avail>@1$dest ;
               assign \>>?$avail>@1$dest = L1e_Port[port].L1_FIFO_IN_Trans_dest_a1;
               logic  \>>?$avail>@1$response_debug ;
               assign \>>?$avail>@1$response_debug = L1e_Port[port].L1_FIFO_IN_Trans_response_debug_a1;
               logic [1:0] \>>?$avail>@1$sender ;
               assign \>>?$avail>@1$sender = L1e_Port[port].L1_FIFO_IN_Trans_sender_a1;
            end
         end
         ...
         if (1) begin : \|rg 
            ...
            //
            if (1) begin : \/trans 
               logic [15:0] \>>?$valid>@1$cyc_cnt ;
               assign \>>?$valid>@1$cyc_cnt = Port_RG_Trans_cyc_cnt_a1[port];
               logic [7:0] \>>?$valid>@1$data ;
               assign \>>?$valid>@1$data = Port_RG_Trans_data_a1[port];
               logic [1:0] \>>?$valid>@1$sender ;
               assign \>>?$valid>@1$sender = Port_RG_Trans_sender_a1[port];
            end
         end
         ...
         if (1) begin : \|ring_out 
            ...
            //
            if (1) begin : \/trans 
               logic [15:0] \>>?$trans_valid>@1$cyc_cnt ;
               assign \>>?$trans_valid>@1$cyc_cnt = L1b_Port[port].L1_RING_OUT_Trans_cyc_cnt_a1;
               logic [7:0] \>>?$trans_valid>@1$data ;
               assign \>>?$trans_valid>@1$data = L1b_Port[port].L1_RING_OUT_Trans_data_a1;
               logic [1:0] \>>?$trans_valid>@1$sender ;
               assign \>>?$trans_valid>@1$sender = L1b_Port[port].L1_RING_OUT_Trans_sender_a1;
            end
         end
      end
      ...
      if (1) begin : \/tb 
         ...
         for (port = 0; port <= 3; port++) begin : \/port 
            ...
            if (1) begin : \|receive1 
               ...
               //
               if (1) begin : \/trans 
                  logic [15:0] \>>>?$accepted>@1$cyc_cnt ;
                  assign \>>>?$accepted>@1$cyc_cnt = L1_Tb_Port[port].L1_RECEIVE1_Trans_cyc_cnt_a1;
                  logic [1:0] \>>>?$accepted>@1$dest ;
                  assign \>>>?$accepted>@1$dest = L1_Tb_Port[port].L1_RECEIVE1_Trans_dest_a1;
                  logic  \>>>?$accepted>@1$request ;
                  assign \>>>?$accepted>@1$request = L1_Tb_Port[port].L1_RECEIVE1_Trans_request_a1;
                  logic  \>>>?$accepted>@1$response ;
                  assign \>>>?$accepted>@1$response = L1_Tb_Port[port].L1_RECEIVE1_Trans_response_a1;
                  logic  \>>>?$accepted>@1$response_debug ;
                  assign \>>>?$accepted>@1$response_debug = L1_Tb_Port[port].L1_RECEIVE1_Trans_response_debug_a1;
                  logic [1:0] \>>>?$accepted>@1$sender ;
                  assign \>>>?$accepted>@1$sender = L1_Tb_Port[port].L1_RECEIVE1_Trans_sender_a1;
               end
            end
            ...
            if (1) begin : \|receive2 
               ...
               //
               if (1) begin : \/trans 
                  logic [15:0] \>>>?$avail>@0$cyc_cnt ;
                  assign \>>>?$avail>@0$cyc_cnt = L1_Tb_Port[port].L1_RECEIVE2_Trans_cyc_cnt_a0;
                  logic [1:0] \>>>?$avail>@0$dest ;
                  assign \>>>?$avail>@0$dest = L1_Tb_Port[port].L1_RECEIVE2_Trans_dest_a0;
                  logic  \>>>?$avail>@0$request ;
                  assign \>>>?$avail>@0$request = L1_Tb_Port[port].L1_RECEIVE2_Trans_request_a0;
                  logic  \>>>?$avail>@0$response ;
                  assign \>>>?$avail>@0$response = L1_Tb_Port[port].L1_RECEIVE2_Trans_response_a0;
                  logic  \>>>?$avail>@0$response_debug ;
                  assign \>>>?$avail>@0$response_debug = L1_Tb_Port[port].L1_RECEIVE2_Trans_response_debug_a0;
                  logic [1:0] \>>>?$avail>@0$sender ;
                  assign \>>>?$avail>@0$sender = L1_Tb_Port[port].L1_RECEIVE2_Trans_sender_a0;
               end
            end
            ...
            if (1) begin : \|send 
               ...
               if (1) begin : \/gen_trans 
                  logic [15:0] \>>>?$valid_in>@1$cyc_cnt ;
                  assign \>>>?$valid_in>@1$cyc_cnt = L1_Tb_Port[port].L1_SEND_GenTrans_cyc_cnt_a1;
                  logic [1:0] \>>>?$valid_in>@1$dest ;
                  assign \>>>?$valid_in>@1$dest = L1_Tb_Port[port].L1_SEND_GenTrans_dest_a1;
                  logic [1:0] \>>>?$valid_in>@1$dest_tmp ;
                  assign \>>>?$valid_in>@1$dest_tmp = L1_Tb_Port[port].L1_SEND_GenTrans_dest_tmp_a1;
                  logic  \>>>?$valid_in>@1$response_debug ;
                  assign \>>>?$valid_in>@1$response_debug = L1_Tb_Port[port].L1_SEND_GenTrans_response_debug_a1;
                  logic [1:0] \>>>?$valid_in>@1$sender ;
                  assign \>>>?$valid_in>@1$sender = L1_Tb_Port[port].L1_SEND_GenTrans_sender_a1;
               end
               ...
               if (1) begin : \/trans_out 
                  logic [15:0] \>>>?$avail>@1$cyc_cnt ;
                  assign \>>>?$avail>@1$cyc_cnt = L1_Tb_Port[port].L1_SEND_TransOut_cyc_cnt_a1;
                  logic [1:0] \>>>?$avail>@1$dest ;
                  assign \>>>?$avail>@1$dest = L1_Tb_Port[port].L1_SEND_TransOut_dest_a1;
                  logic  \>>>?$avail>@1$response_debug ;
                  assign \>>>?$avail>@1$response_debug = L1_Tb_Port[port].L1_SEND_TransOut_response_debug_a1;
                  logic [1:0] \>>>?$avail>@1$sender ;
                  assign \>>>?$avail>@1$sender = L1_Tb_Port[port].L1_SEND_TransOut_sender_a1;
               end
            end
         end
